    ...
    <script>
        ...
        $(document).ready(function(){
            $('.newroom-btn').click(function(){
                ...
                if(btnclicked == true){
                    document.querySelector('.newroom-btn').innerHTML = 'Add new Room';
                    btnclicked = false;
                }else{
                    document.querySelector('.newroom-btn').innerHTML = 'Show Room Shedule';
                    btnclicked = true;
                }
        ...
        <div class="container loader">
            ...
            <div class="row span">
                ...
                <div class="contianer staff ">
                    ...
                    <h1 class="form-topic">New Shedule</h1>
                        ...
                        " placeholder="Shedule ID. Don't fill when creating new shedule"> <br></br>
                            ...
                            <select name="room_id" class="sec-role" placeholder="Select Room ID" value="">
                            <option value="<?php if(isset($_SESSION['r_id'])){
                                echo $_SESSION['r_id'];
                            }else{echo '';}?>">
                            <?php if(isset($_SESSION['r_id'])){
                                ...
                                $_SESSION['r_id'] = 'Select Room ID';
                                echo $_SESSION['r_id'];
                                unset($_SESSION['r_id']);
                            }?></option>
                           
                           <?php
                                $sql = "SELECT r_id,r_type FROM room";
                        ...
                        <button type="button" class="create-btn newroom-btn" name="addshedule">Add New Room</button>
                    </form>
                    </div>
                    <?php
                    if(isset($_POST['addshedule'])){
                        ...
                        $sql = "INSERT INTO room_assignment(r_id,s_id,sheduled_date,sheduled_shift) VALUES({$room_id},{$staff},'{$s_date}','{$shift}');";
                        ...
                        if($conn->query($sql)===true){
                            ...
                            echo "<script>
                                alert('Room sheduled for the staff member');
                            ...
                            echo "<script>
                                alert('Cannot shedule this room for this staff member.');
                                location.replace('room.php');
                            </script>";
                    ...
                    if(isset($_POST['updateshedule'])){
                        ...
                        $s_date = $_POST['s_date'];
                        $staff = $_POST['staff'];
                        $sql = "UPDATE room_assignment SET r_id = {$room_id} , s_id = {$staff}, sheduled_date = '{$s_date}', sheduled_shift='{$shift}' WHERE ra_id = {$id}";
                            ...
                            <tbody>
                                <?php
                                    function filltable($conn){
                                        $sql = "SELECT * FROM room_assignment r, staff s WHERE r.s_id = s.s_id";
                                        ...
                                        if($result->num_rows>0){
                                            ...
                                            <td><a class='showbtn' href='helper.php?roomshowid={$row['ra_id']}' ><i class='fa-regular fa-eye'></i></a>
                                            <a class='deletebtn' href='helper.php?roomdelid={$row['ra_id']}' onclick='delconfirmation()'><i class='fa-solid fa-trash'></i></a>
                                            </td>
                                            </tr>";
                                            }
                                        }
                ...
                <div class="container span" id="new-room-id">
                    ...
                    <h1 class="form-topic">Add New Room</h1>
                        <input type="text" name="room_id" placeholder="Room_id. Don't need to fill">
                        <input type="text" name="room_capacity" placeholder="Room Capacity">
                        <select name="room_type" class="sec-role" id="" aria-placeholder="Select Room Type">
                        ...
                        <select name="room_availability" class="sec-role" id="">
                            <option value="">--Select Room Availability--</option>
                            <option value="available">Available</option>
                            <option value="not available">Not Available</option>
                        </select>

                        <button type="submit" class="addroomnew-btn" name="addnewroom">Add New Room</button>
                    </form>
                </div>
            </div>
        <?php
            if(isset($_POST['addnewroom'])){
                ...
                $sql = "INSERT INTO room(r_type,r_capacity,r_availability)VALUES('{$type}',{$capacity},'{$availability}');";
